...
public class DataHelper extends SQLiteOpenHelper {
    ...
    private static final int DATABASE_VERSION = 1; // [cite: 1183]
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("create table bookings(no integer primary key autoincrement, " +
                "sport text, court text, date text, time text);"); // [cite: 1195]
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int old, int next) {} // [cite: 1197]
}